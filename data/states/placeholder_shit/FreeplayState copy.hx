...
//		CustomFadeTransition.nextCamera = camText;
		...
		bg = new FlxSprite();
		bg.frames = Paths.getSparrowAtlas('menus/freeplay/freeplaymenu/mainbgAnimate');
...
//			bg.frames = Paths.getSparrowAtlas('freeplaymenu/classicbgAnimate');
		...
		var darkportrait:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/freeplay/portraits/ron'));
		darkportrait.scale.set(0.5,0.5);
		darkportrait.updateHitbox();
//		darkportrait.antialiasing = ClientPrefs.globalAntialiasing;
		add(darkportrait);
		darkportrait.screenCenter();
...
//		portrait.antialiasing = ClientPrefs.globalAntialiasing;
		...
		bar.frames = Paths.getSparrowAtlas('menus/freeplay/bar');
...
//		curDifficulty = Math.round(Math.max(0, CoolUtil.defaultDifficulties.indexOf(lastDifficultyName)));
	...
	static var vocals:FlxSound = null;
...
//		super.update(elapsed);
	...
	function changeDiff(change:Int = 0, isMuda:Bool = false)
	{
		...
		if (curDifficulty >= CoolUtil.difficulties)
			curDifficulty = 0;

//		lastDifficultyName = CoolUtil.difficulties[curDifficulty];

		#if !switch
		var songName:String = songs[curSelected].displayName;
...
//		PlayState.storyWeek = songs[curSelected].WeekData;
...
*/
	...
	function updateportrait() {
		for (i in songs){
		portrait.loadGraphic(Paths.image('freeplayportraits/'+ i.name));
		portrait.scale.set(0.5,0.5);
		portrait.updateHitbox();
		portrait.screenCenter();
		...
		{
			portraitOverlay.loadGraphic(Paths.image('freeplayportraits/'+ i.name+'-over'));
			portraitOverlay.scale.set(0.5,0.5);
			portraitOverlay.updateHitbox();
			portraitOverlay.screenCenter();
